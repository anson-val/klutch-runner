...
import com.example.classes.compilers.PythonPass
import com.example.classes.executors.GCCExecutor
import com.example.classes.executors.JVMExecutor
import com.example.classes.executors.PythonExecutor
...
fun getJudge(language: String): Judge =
    when (language) {
        "kotlin" -> Judge(
            KotlinCompiler(ConfigLoader.config.runner.dockerWorkspace),
            JVMExecutor(ConfigLoader.config.runner.dockerWorkspace)
        )

        "java" -> Judge(
            JavaCompiler(ConfigLoader.config.runner.dockerWorkspace),
            JVMExecutor(ConfigLoader.config.runner.dockerWorkspace)
        )

        "c" -> Judge(
            GCCCompiler(ConfigLoader.config.runner.dockerWorkspace),
            GCCExecutor(ConfigLoader.config.runner.dockerWorkspace)
        )

        "python" -> Judge(
            PythonPass(ConfigLoader.config.runner.dockerWorkspace),
            PythonExecutor(ConfigLoader.config.runner.dockerWorkspace)
        )
